...
describe('concat function', () => {
  it('should return SingleString from Single and String', () => {
    const result = concat('Single', 'String');
    expect(result).to.equal('SingleString');
  });

  it('should throw an error if either string is empty', () => {
    expect(() => { concat('Single', '') }).to.throw('Both strings required')
  });
});